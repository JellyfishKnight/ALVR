...
pub struct StreamRenderer {
    ...
    last_foveation_centers: Cell<Option<[Vec2; 2]>>,
}
...
impl StreamRenderer {
    ...
    pub fn new(
        ...
        constants.extend([
            ...
            (
                "SHOW_FOVEATION_DIAGNOSTIC",
                cfg!(feature = "foveation-diagnostics").into(),
            ),
        ]);
        ...
        Self {
            ...
            foveated_encoding,
            foveation_buffer,
            last_foveation_centers: Cell::new(None),
        }
    }
    ...
    pub fn render(
        ...
        if let Some(config) = &self.foveated_encoding
            && self.last_foveation_centers.get() != foveation_centers
        {
            let centers = if let Some(centers) = foveation_centers {
                FoveationCenterShifts::EncoderAligned(centers)
            } else {
                FoveationCenterShifts::Requested(static_foveation_center_shifts(config))
            };
            ...
            self.last_foveation_centers.set(foveation_centers);
        }
...
fn static_foveation_center_shifts(config: &FoveatedEncodingConfig) -> [Vec2; 2] {
    // Per-frame center metadata is optional, so its absence retains the legacy static transform.
...
fn foveated_encoding_data(
    ...
    let center_shifts = match center_shifts {
        ...
        FoveationCenterShifts::EncoderAligned(center_shifts) => center_shifts.map(|center_shift| {
            glam::vec2(
...
fn align_center_shift(center_shift: f32, edge_size_aligned: f32, edge_ratio: f32) -> f32 {
    ...
    let aligned = (center_shift / alignment_step).ceil() * alignment_step;
