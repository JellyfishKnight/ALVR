            ...
            ConnectionAcceptedInfo {
                ...
                streaming_capabilities: Some(
                    ...
                    .with_ext(VideoStreamingCapabilitiesExt {
                        foveation_center_metadata: capabilities.foveation_center_metadata,
                    }),
                ),
            },
        )))
        ...
        move || {
            ...
            while is_streaming(&ctx) {
                ...
                if !stream_corrupted || !settings.connection.avoid_video_glitching {
                    // Frame metadata must be enqueued before calling the decoder callback. There
                    // is no problem if the callback fails.
                    {
                        let queue = &mut ctx.video_frame_metadata_queue.lock();
                        queue.push_back((
                            header.timestamp,
                            VideoFrameMetadata {
                                view_params: header.global_view_params,
                                foveation_centers: header.foveation_centers,
                            },
                        ));
